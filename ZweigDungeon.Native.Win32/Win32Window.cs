...
using ZweigDungeon.Native.Win32.Prototypes;
...
public sealed class Win32Window : IDisposable, IPlatformWindow
{
	...
	public void Update()
	{
		...
		if (IsAvailable())
		{
			...
			if (!IsFocused())
			{
				MsgWaitForMultipleObjects(0u, Array.Empty<IntPtr>(), false, 100, Win32QueueStatusFlags.Allinput);
			}
		}
	}
	...
	private void NotifyWindowUpdate()
	{
		...
		WrapError(() => m_sync.Execute(() =>
		{
			try
			{
				...
				m_messageBus.Broadcast<IWindowListener>(listener => listener.WinodwUpdateFrame(this));
